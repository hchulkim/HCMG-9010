...
itt1 <- feols(hosp_any_12m ~ treatment + hhsize_12m + draw_lottery + hosp_any_0m + er_any_0m, weights = ~ weight_12m, data = data, cluster = ~ household_id)

itt2 <- feols(er_any_12m ~ treatment + hhsize_12m + draw_lottery +hosp_any_0m + er_any_0m, weights = ~ weight_12m, data = data, cluster = ~ household_id)
...
I will use first use the "ever on medicaid" as the measures of coverage.
...
data1 <- haven::read_dta("../data/OHIE_Public_Use_Files/OHIE_Data/oregonhie_descriptive_vars.dta")
data2 <- haven::read_dta("../data/OHIE_Public_Use_Files/OHIE_Data/oregonhie_stateprograms_vars.dta")
data3 <- haven::read_dta("../data/OHIE_Public_Use_Files/OHIE_Data/oregonhie_survey12m_vars.dta")

data <- data %>% 
  left_join(data2, by="person_id") %>% 
  left_join(data3, by="person_id") %>% 
  mutate(medicaid = ohp_all_ever_matchn_30sep2009, drug = rx_any_12m, outpatient = doc_any_12m, ER_visit = er_any_12m, inpatient = hosp_any_12m)

result1 <- feols(drug ~ medicaid, data = data)
result2 <- feols(outpatient ~ medicaid, data = data)
result3 <- feols(ER_visit ~ medicaid, data = data)
result4 <- feols(inpatient ~ medicaid, data = data)


etable(result1, result2, result3, result4, style.tex = style.tex("aer"),
  digits = "r3", drop = "Constant", fitstat = ~ n + r2,
